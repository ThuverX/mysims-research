...
> Check out the Kingdom version [here](./Kingdom.mdx).
...
<ObjectDef> <!-- Seems to depend on the folder its in. Definitely want to check that out -->
...
#### Test()
Before we see our interaction when standing in front of our object, or NPC (or ourselves if you have Debug Enabler downloaded!), the game checks if the interaction should even be shown at all. "Can we fire it? Is this the correct candidate?" Is basically what it asks.
...
#### Run()
This is where the magic happens. Here, we make the interaction usable and animatable (or just have our sim animate!)... or for a super simple test, even 'talk' with 'DialogMessage()'.
...
```lua
    ...
    -- Spawn Object:
    SpawnObject("MineralDispensor", "ObjectDefs/Dispenser_Generic_Def.xml", ObjectTypes.eHarvestingNode, x, y, z, 0)  -- Params: className? or ModelName?, def xml of item, object type, x, y, z, degrees
    ...
    local _,yRot,_ = GetGameObjectOrientation(obj) -- Params: GameObject.
    ...
    SnapToPositionRotation( self, x, y, z, 360 ) -- Item to rotate, x,y,z, 360 degrees.
    ...
    -- Create fx effect! Make sure to attach it to an nil object inside "constructor()"!
    ...
    if sim:GetPrimaryInterest() == InterestType["kFood"] then
        -- Do something if sim's primary interest is food.
    end
...
however, referring back to my mention of "complexer objects", the usage of `PlayAnimBlocking` is primarily used. It's simply playing a solo animation outside of the interaction def, and therefore not needing the need of our interaction def's animation. So I was thinking way too complicated about this :p
